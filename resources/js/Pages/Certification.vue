        ...
        <div class="content">
            <div class="content-header">
                <div class="mt-auto mb-0">
                    <div class="text-lg font-semibold">{{ $t('base.certification') }}</div>
                </div>
                <div class="mt-auto mb-0">
                    <el-breadcrumb separator="/">
                        <el-breadcrumb-item :to="{ path: '/dashboard' }">{{ $t('base.dashboard') }}
                        </el-breadcrumb-item>
                        <el-breadcrumb-item>{{ $t('base.certification') }}</el-breadcrumb-item>
                    </el-breadcrumb>
                </div>
            </div>
            ...
            <el-dialog id="modalForm" v-model="formShow" :title="formTitle" class="!sm:w-full !w-1/3 rounded-lg"
                ...
                <el-form label-position="top" ref="formRef" :model="form" :rules="formRules" @submit.prevent="onSubmit">
                    ...
                    <el-form-item label="Tanggal Berlaku" prop="date_exp">
                        <el-input v-model="form.date_exp" />
                    </el-form-item>
...
<script setup>
    ...
    const fetchData = async ({
        ...
        const response = await axios.get("/certification/data", {
            params: queryParams,
        });
    ...
    const openModal = () => {
        formTitle.value = `${ t('common.create') } ${ t('base.certification') }`;
        formShow.value = true;
        form.value.id = null;
        form.value.name = null;
        form.value.date_exp = null;
        form.value.image = null;
    }

    const onEdit = (data, locale = 'id') => {
        formTitle.value = `${ t('common.edit') } ${ t('base.certification') }`;
        formShow.value = true;
        form.value.id = data.id;
        form.value.name = data.name;
        form.value.date_exp = data.date_exp;
        ...
        formRef.value.validate(async (valid) => {
            if (valid) {
                ...
                try {
                    ...
                    const url = form.value.id ? `/certification/${form.value.id}/update` :
                        '/certification/store';
                    const method = 'post';
    ...
    const onDelete = (id) => {
        ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
            ...
            axios.delete(`/product/certification/${id}/delete`).then(() => {
                refetch();
                ElMessage({
                    type: 'success',
