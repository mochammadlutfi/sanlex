            ...
            <el-card body-class="!p-0" class="!rounded-lg !shadow-md !mb-10">
                <div class="flex justify-between items-center p-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <el-button :tag="Link" href="/project/contact/create" type="primary">
                ...
                <el-skeleton :loading="isLoading" animated>
                    ...
                    <template #default>
                        <el-table class="min-w-full" :data="data.data" @sort-change="sortChange"
                            ...
                            <el-table-column prop="name" label="Nama" width="200" header-align="center"/>
                            <el-table-column label="Alamat" header-align="center">
                                <template #default="scope">
                                    <div>{{ scope.row.address }}</div>
                                </template>    
                            </el-table-column>
                            <el-table-column prop="pic" label="PIC" width="200" header-align="center">
                                <template #default="scope">
                                    <div>{{ scope.row.pic_name }}</div>
                                    <div>+{{ scope.row.pic_phone }}</div>
                                </template>    
                            </el-table-column>
                                        ...
                                        <template #dropdown>
                                            <el-dropdown-menu>
                                                <el-dropdown-item class="flex justify-between">
                                                    <Link :href="`/project/contact/${ scope.row.id }/edit`">
...
<script setup>
    ...
    const fetchData = async ({
        ...
        const response = await axios.get("/project/contact/data", {
            params: queryParams,
        });
    ...
    const {
        ...
        queryKey: ['projectContactList', params.value], // Query key unik
        queryFn: fetchData,
    ...
    const onDelete = (id) => {
        ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
            ...
            axios.delete(`/project/contact/${id}/delete`).then(() => {
                refetch();
                ElMessage({
                    type: 'success',
