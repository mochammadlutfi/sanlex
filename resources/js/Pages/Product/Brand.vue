        ...
        <div class="content">
            <div class="content-header">
                <div class="mt-auto mb-0">
                    <div class="text-lg font-semibold">{{ $t('base.brand') }}</div>
                </div>
                <div class="mt-auto mb-0">
                        ...
                        <el-table class="min-w-full" :data="data.data" @sort-change="sortChange"
                            ...
                            <el-table-column :label="$t('common.image')">
                                <template #default="scope">
                                    <el-image 
                                        class="!w-24 !h-14 rounded-md me-2"
                                        :src="scope.row.image"
...
<script setup>
    ...
    const fetchData = async ({
        ...
        const response = await axios.get("/product/brand/data", {
            params: queryParams,
        });
    ...
    const openModal = () => {
        formTitle.value = `${ t('common.create') } ${ t('base.packaging') }`;
        formShow.value = true;
        form.value.id = null;
    ...
    const onEdit = (data) => {
        formTitle.value = `${ t('common.edit') } ${ t('base.packaging') }`;
        formShow.value = true;
        ...
        formRef.value.validate(async (valid) => {
            if (valid) {
                ...
                try {
                    ...
                    const url = form.value.id ? `/product/brand/${form.value.id}/update` :
                        '/product/brand/store';
